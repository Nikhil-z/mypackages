...
import com.konka.custom.view.CaErrorType.CA_NOTIFY;
...
public class CaViewSmallWindow {
	...
	public void showNoSignalTextView(int resid) {
		...
		Log.d("smallwin", "smallwin showNoSignalTextView resid " + resid + " " + caContext);
		...
		if((iShowNoProgMsg ==1)&&(0 == resid)){
			...
			if(null == pProgManager.getCurrverntProgListInfo())
			{
				...
				Log.d("smallwin", "now no prog " + caContext);
			}
			else{
				iShowNoProgMsg = 0;
			}
		...
		if (noSignalTextView == null) {
			...
			noSignalTextView = new TextView(caContext);
